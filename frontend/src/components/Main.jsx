...
// Выносим хук на верхний уровень
const useFoodImages = () => {
    const foodImages = [
...
export default function Main() {
    ...
    // Хук используем на верхнем уровне компонента
    ...
    const navigateToProduct = (product, img) => {
        navigate(`/product`, { 
            state: { 
                product: {
                    ...product,
                    // Добавляем недостающие поля для ProductCard
                    title: product.name,
                    about: product.description || 'Описание товара',
                    mass: product.weight || 500,
                    img: img,
                    calories: product.calories || 250,
                    fats: product.fats || 15,
                    proteins: product.proteins || 20,
                    carbohydrates: product.carbohydrates || 10,
                    producers: product.offers || [
                        { 
                            name: product.best_offer?.merchant_name || 'Магазин', 
                ...
                >
                    <div className="relative overflow-hidden">
                        <img
                            ...
                            src={imageUrl}
                            alt={product.name}
                            onClick={() => navigateToProduct(product, imageUrl)}
                            onError={(e) => {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            ...
                            <div className="">
                                ...
                                <p className="text-xs text-gray-400 block">
                                    магазин: {product.best_offer?.merchant_name || 'Не указан'}
                                </p>
                                <p className="text-xs text-gray-400 block">
